...
(function (global, factory) {
    ...
    disable() {
      ...
      setTimeout(() => {
        ...
        if (this.name === "weboji") {
          ...
          ((_this$handsfree$debug = this.handsfree.debug.context[this.name]) ===
            null || _this$handsfree$debug === void 0
            ? void 0
            : _this$handsfree$debug.clearRect) &&
            this.handsfree.debug.context[this.name].clearRect(
              0,
              0,
              this.handsfree.debug.$canvas[this.name].width,
              this.handsfree.debug.$canvas[this.name].height
            );
        } // Stop if all models have been stopped

        let hasRunningModels = Object.keys(this.handsfree.model).some(
          (model) => this.handsfree.model[model].enabled
        );
    ...
    runPlugins() {
      ...
      if (Object.keys(this.data).length) {
        this.plugins.forEach((name) => {
          ...
          this.handsfree.plugin[name].enabled &&
            ((_this$handsfree$plugi = this.handsfree.plugin[name]) === null ||
            _this$handsfree$plugi === void 0
              ? void 0
              : _this$handsfree$plugi.onFrame(this.handsfree.data));
        });
      }
    }
  }
        ...
        function (t, e, n) {
          ...
          var o = {
              ...
              getPoints: function (t) {
                return (
                  void 0 !== i(this.pointsMapping[t]) && this.pointsMapping[t]
                );
              },
            },
            a = {
                    ...
                    try {
                      for (O.s(); !(b = O.n()).done; ) {
                        var M = b.value,
                          ...
                          _ = this.calculateFingerDirection(
                            C,
                            R,
                            A,
                            n[M].slice(S)
  ...
  class HandsModel extends BaseModel {
    ...
    loadDependencies(callback) {
      ...
      if (this.handsfree.config.isClient) {
        this.loadDependency(
          `${this.handsfree.config.assetsPath}/@mediapipe/drawing_utils.js`,
          () => {
      ...
      this.loadDependency(
        `${this.handsfree.config.assetsPath}/@mediapipe/hands/hands.js`,
        () => {
          // Configure model
          ...
          this.handsfree.getUserMedia(() => {
            ...
            if (!this.handsfree.mediapipeWarmups.isWarmingUp) {
              ...
              this.handsfree.on("mediapipeWarmedUp", () => {
                if (
                  !this.handsfree.mediapipeWarmups.isWarmingUp &&
                  !this.handsfree.mediapipeWarmups[this.name]
                ) {
                  this.warmUp(callback);
                }
              });
    ...
    debug(results) {
      ...
      this.handsfree.debug.context.hands.clearRect(
        0,
        0,
        this.handsfree.debug.$canvas.hands.width,
        this.handsfree.debug.$canvas.hands.height
      ); // Draw skeletons

      if (results.multiHandLandmarks) {
        for (const landmarks of results.multiHandLandmarks) {
          drawConnectors(
            this.handsfree.debug.context.hands,
            landmarks,
            HAND_CONNECTIONS,
            {
              color: "#00FF00",
    ...
    updateGestureEstimator() {
      ...
      this.gestures.forEach((name) => {
        ...
        if (
          !this.handsfree.gesture[name].compiledDescription &&
          this.handsfree.gesture[name].enabled
        ) {
      ...
      activeGestures.forEach((gesture) => {
        gestureDescriptions.push(
          this.handsfree.gesture[gesture].compiledDescription
        );
      });
    ...
    getGesture() {
      ...
      this.data.landmarks.forEach((landmarksObj, hand) => {
        if (this.data.landmarksVisible[hand]) {
          ...
          for (let i = 0; i < 21; i++) {
            landmarks.push([
              landmarksObj[i].x * window.outerWidth,
              landmarksObj[i].y * window.outerHeight,
              0,
            ]);
          } // Estimate
          ...
          if (estimate.gestures.length) {
            gestures[hand] = estimate.gestures.reduce((p, c) => {
              const requiredConfidence =
                this.handsfree.gesture[c.name].confidence;
              return c.confidence >= requiredConfidence &&
          ...
          if (gestures[hand].name) {
            const requiredConfidence =
              this.handsfree.gesture[gestures[hand].name].confidence;
  ...
  class FacemeshModel extends BaseModel {
    ...
    loadDependencies(callback) {
      ...
      this.loadDependency(
        `${this.handsfree.config.assetsPath}/@mediapipe/face_mesh/face_mesh.js`,
        () => {
          // Configure model
    ...
    debug(results) {
      ...
      this.handsfree.debug.context.facemesh.clearRect(
        0,
        0,
        this.handsfree.debug.$canvas.facemesh.width,
        this.handsfree.debug.$canvas.facemesh.height
      );

      if (results.multiFaceLandmarks) {
        for (const landmarks of results.multiFaceLandmarks) {
          drawConnectors(
            this.handsfree.debug.context.facemesh,
            landmarks,
  ...
  class PoseModel extends BaseModel {
    ...
    loadDependencies(callback) {
      ...
      this.loadDependency(
        `${this.handsfree.config.assetsPath}/@mediapipe/pose/pose.js`,
        () => {
    ...
    debug(results) {
      this.handsfree.debug.context.pose.clearRect(
        0,
        0,
        this.handsfree.debug.$canvas.pose.width,
        this.handsfree.debug.$canvas.pose.height
      );

      if (results.poseLandmarks) {
        drawConnectors(
          this.handsfree.debug.context.pose,
  ...
  class HandposeModel extends BaseModel {
    ...
    loadDependencies(callback) {
      this.loadDependency(
        `${this.handsfree.config.assetsPath}/three/three.min.js`,
        () => {
          this.loadDependency(
            `${this.handsfree.config.assetsPath}/@tensorflow/tf-core.js`,
            () => {
              this.loadDependency(
                `${this.handsfree.config.assetsPath}/@tensorflow/tf-converter.js`,
                () => {
                  this.loadDependency(
                    `${this.handsfree.config.assetsPath}/@tensorflow/tf-backend-${this.handsfree.config.handpose.backend}.js`,
                    () => {
                      this.loadDependency(
                        `${this.handsfree.config.assetsPath}/@tensorflow-models/handpose/handpose.js`,
                        () => {
                          this.handsfree.getUserMedia(async () => {
                            await window.tf.setBackend(
                              this.handsfree.config.handpose.backend
                            );
                            this.api = await handpose.load(
                              this.handsfree.config.handpose.model
                            );
                            this.setup3D();
                            callback && callback(this);
                            this.dependenciesLoaded = true;
    ...
    async getData() {
      ...
      const predictions = await this.api.estimateHands(
        this.handsfree.debug.$video
      );
    ...
    setup3D() {
      ...
      this.three.arrow = new window.THREE.ArrowHelper(
        this.three.raycaster.ray.direction,
        this.three.raycaster.ray.origin,
        300,
        0xff0000
      );
    ...
    updateMeshes(hand) {
      for (
        ...
        this.normalized[i] = [
          this.handsfree.normalize(
            p0.x,
            this.handsfree.debug.$video.videoWidth / -2,
            this.handsfree.debug.$video.videoWidth / 2
          ),
          this.handsfree.normalize(
            p0.y,
            this.handsfree.debug.$video.videoHeight / -2,
            this.handsfree.debug.$video.videoHeight / 2
          ),
    ...
    updateCenterPalmMesh(hand) {
      ...
      this.three.centerPalmObj.rotation.x =
        this.three.meshes[12].rotation.x - Math.PI / 2;
    ...
    getGesture() {
      ...
      if (this.data.landmarks && this.gestureEstimator) {
        const estimate = this.gestureEstimator.estimate(
          this.data.landmarks,
          7.5
        );
  ...
  class WebojiModel extends BaseModel {
    ...
    loadDependencies(callback) {
      ...
      this.loadDependency(
        `${this.handsfree.config.assetsPath}/jeeliz/jeelizFaceTransfer.js`,
        () => {
          const url =
            this.handsfree.config.assetsPath +
            "/jeeliz/jeelizFaceTransferNNC.json";
          this.api = window.JEEFACETRANSFERAPI;
          fetch(url)
            .then((model) => model.json()) // Next, let's initialize the weboji tracker API
            .then((model) => {
              this.api.init({
    ...
    getStates() {
      ...
      state.smirk =
        ...
        (!state.smileRight && state.smileLeft);
      state.pursed =
        morphs[7] > this.handsfree.config.weboji.morphs.threshold.mouthRound; // Eyebrows
  ...
  class Plugin {
    constructor(plugin, handsfree) {
      ...
      let handsfreePluginConfig =
        (_handsfree$config = handsfree.config) === null ||
        _handsfree$config === void 0
          ? void 0
          : (_handsfree$config$plu = _handsfree$config.plugin) === null ||
            _handsfree$config$plu === void 0
          ? void 0
          : _handsfree$config$plu[plugin.name];
  ...
  class BaseGesture {
    constructor(gesture, handsfree) {
      ...
      let handsfreeGestureConfig =
        (_handsfree$config = handsfree.config) === null ||
        _handsfree$config === void 0
          ? void 0
          : (_handsfree$config$ges = _handsfree$config.gesture) === null ||
            _handsfree$config$ges === void 0
          ? void 0
          : _handsfree$config$ges[gesture.name];
  ...
  var pluginFacePointer = {
    ...
    onEnable() {
      ...
      (_this$$pointer = this.$pointer) === null || _this$$pointer === void 0
        ? void 0
        : _this$$pointer.classList.remove("handsfree-hidden");
    },
    ...
    onFrame({ weboji }) {
      ...
      x +=
        z *
        Math.tan(
          weboji.rotation[1] + (this.config.offset.yaw * Math.PI) / 180
        ) *
        this.config.speed.x;
      y +=
        z *
          ...
          this.config.speed.y -
        window.outerHeight; // Add offsets
    ...
    onDisable() {
      ...
      (_this$$pointer2 = this.$pointer) === null || _this$$pointer2 === void 0
        ? void 0
        : _this$$pointer2.classList.add("handsfree-hidden");
    },
  };
  ...
  var pluginFaceClick = {
    ...
    onFrame({ weboji }) {
      ...
      else if (this.mouseUp) event = weboji.pointer.state = "mouseup";
      else event = "mousemove"; // Actually click something (or focus it)
      ...
      if ($el) {
        ...
        if (
          ...
          ["INPUT", "TEXTAREA", "BUTTON", "A"].includes($el.nodeName)
        )
          $el.focus(); // Click
  ...
  var pluginFaceScroll = {
    ...
    onFrame({ weboji }) {
      ...
      if (
        weboji.pointer.y >
        bounds.bottom - this.config.vertScroll.scrollZone
      ) {
        this.$target.scrollTo(
          0,
          scrollTop -
    ...
    maybeSelectNewTarget() {
      ...
      this.$target.scrollTo(
        0,
        curScrollTop + this.config.vertScroll.scrollSpeed
      );
    ...
    getTargetScrollTop() {
      ...
      return (
        ((_this$$target = this.$target) === null || _this$$target === void 0
          ? void 0
          : _this$$target.scrollY) ||
        ((_this$$target2 = this.$target) === null || _this$$target2 === void 0
          ? void 0
          : _this$$target2.scrollTop) ||
        0
      );
    },
    ...
    maybeSetTarget(weboji) {
      if (weboji.pointer.state === "mouseDown" && weboji.pointer.$target) {
        this.selectTarget(
          this.recursivelyFindScrollbar(weboji.pointer.$target)
        );
      }
    },
  ...
  var potTargetFrom = "";
  var secs = 30000;
  var successive_points = 0;
  var nb_folder_dragged = 0;
  let varCacheCopy = "";

  function doSomething(pointer) {
    let $potTarget = document.elementFromPoint(pointer.x, pointer.y);
    // Exemple de $potTarget : <div id="myDiv2"></div>
    if ($potTarget !== undefined && $potTarget !== null) {
      let para = $potTarget.childNodes[1];
      // Exemple de para: <p id="paragraphe1">
      if (para !== undefined && para.nodeName === "P") {
        potTargetFrom = para.cloneNode(true);
      }
    }
  }
  ...
  const explosion = () => {
    return wait(0).then(() =>
      document
        .getElementById("deskcase")
        .setAttribute("src", "./img/explosion.gif")
    );
  };
  ...
  const overflowing_cabinet = () => {
    return wait(1000).then(() => {
      ...
      // document.getElementById("deskcase").style.width = "500px";
      // document.getElementById("deskcase").setAttribute("height", "auto");
      document
      ...
      document
        .getElementById("passerTroisiemeExercice")
        .classList.remove("passerTroisiemeExerciceStyleVerrouille");
      document
        .getElementById("passerTroisiemeExercice")
        .classList.add("passerTroisiemeExerciceStyleDeverrouille");
      ;
    });
  };
  ...
  async function myFuncAnimation() {
    // await fillingCabinet3();
    await explosion();
    await overflowing_cabinet();
  }
      ...
      setTimeout(() => {
        ...
        hands.pointer.forEach((pointer, n) => {
          ...
          if (hands.pinchState[n]?.[0] === "start") {
            successive_points++;
            // console.log(successive_points);
            let $potTarget = document.elementFromPoint(pointer.x, pointer.y);
            if ($potTarget !== undefined && $potTarget !== null) {
              if ($potTarget.nodeName === "DIV") {
                if ($potTarget.id === "divpasserSecondExercice") {
                  document.getElementById("divpasserSecondExercice").click();
                }
              }
              if ($potTarget.nodeName === "BUTTON") {
                if ($potTarget.id === "effacerField") {
                  // c'est le bouton de effacer alors on efface la valeur du champ
              ...
              if ($potTarget.nodeName === "A") {
                if ($potTarget.id === "envoyerButtonCard") {
                  document.getElementById("envoyerButtonCard").click();
                } else if ($potTarget.id === "ApasserSecondExercice") {
                  document.getElementById("divpasserSecondExercice").click();
                }
                varCacheCopy = "";
              }

              if (successive_points >= 2) {
                if ($potTarget.getAttribute("data-copyable") === "true") {
                  
                  if (varCacheCopy === "") {
                    potTargetFrom = $potTarget.cloneNode(true);
                    console.log("potTargetFrom => ", potTargetFrom);
                    document
                      .getElementById($potTarget.id)
                      .setAttribute("data-copyable", false);
                    varCacheCopy = "full";

                    if (potTargetFrom.id != "card_number") {
                      document.getElementById(
                        potTargetFrom.id
                      ).style.opacity = 0;
                      console.log(potTargetFrom.id);
                    }
                  }
                                      if (potTargetFrom.id != "card_number") {
                                        document.getElementById(
                  ...
                  var elemAlert = document.getElementById("copiedSignal");
                  console.log("elemAlert :" + elemAlert);
                  elemAlert.classList.add("show");
                  setTimeout(function (text) {
                    elemAlert.classList.remove("show");
                  }, 1000);
                  
                  let datafiedType = $potTarget.getAttribute("data-fieldType");
                  potTargetFrom.setAttribute("data-fieldType", datafiedType);
                }
              }
            }
          }
          ...
          // Start scroll
          if (hands.pinchState[n]?.[1] === "start") {
            if (successive_points < 2) {
              successive_points = 0;
            } else {
              let potTargetTo = document.elementFromPoint(pointer.x, pointer.y);
              if (
                potTargetTo !== undefined &&
                potTargetTo !== null &&
                potTargetFrom !== undefined &&
                potTargetFrom !== null
              ) {
                if (
                  potTargetTo.getAttribute("data-fieldType") ===
                  potTargetFrom.getAttribute("data-fieldType")
                ) {
                  if (potTargetTo.nodeName === "INPUT") {
                    potTargetTo.value = potTargetFrom.innerHTML;
                    document
                      .getElementById("envoyerButtonCard")
                      .classList.remove("cardMyDiv3EnvoyerDeactivated");
                    document
                      .getElementById("cardMyDiv3EnvoyerActivated")
                      .classList.add("btn-primary");
                    document
                      .getElementById("envoyerButtonCard")
                      .style.removeProperty("pointer-events");
                  } else if (
                    potTargetTo.getAttribute("data-pastable") === "true" &&
                    varCacheCopy !== ""
                  ) {
                    nb_folder_dragged++;
                    switch (nb_folder_dragged) {
                      case 1:
                        document
                        .getElementById("deskcase")
                        .setAttribute("src", "./img/final1case.png");
                        varCacheCopy = "";
                        break;
                      case 2:
                        document
                      ...
                      case 3:
                        ...
                        document.getElementById("messageBazard").style.display =
                          "inline";
                        document.getElementById(
                          "dernierExerciceButton"
                        ).className = document
                          .getElementById("dernierExerciceButton")
                          .className.replace(/(?:^|\s)inactiveLink(?!\S)/g, "");
                        varCacheCopy = "full";
                        break;
                      default:
                        break;
                    }
                    potTargetTo.appendChild(potTargetFrom);
                  }
                }
              }
              successive_points = 0;
          ...
          if (hands.pinchState[n]?.[0] === "held" && this.$target[n]) {
            ...
            this.handsfree.TweenMax.to(this.tweenScroll[n], 1, {
              x:
                this.origScrollLeft[n] -
                (hands.origPinch[n][0].x - hands.curPinch[n][0].x) * width,
              y:
                this.origScrollTop[n] +
                (hands.origPinch[n][0].y - hands.curPinch[n][0].y) * height,
              overwrite: true,
              ease: "linear.easeNone",
              immediateRender: true,
            });
    ...
    onFrame({ hands }) {
      ...
      setTimeout(() => {
        ...
        hands.pointer.forEach((pointer, n) => {
          ...
          if (
            ((_hands$pinchState$n = hands.pinchState[n]) === null ||
            _hands$pinchState$n === void 0
              ? void 0
              : _hands$pinchState$n[0]) === "start"
          ) {
            let $potTarget = document.elementFromPoint(pointer.x, pointer.y);
            this.$target[n] = this.getTarget($potTarget);
            this.tweenScroll[n].x = this.origScrollLeft[n] =
              this.getTargetScrollLeft(this.$target[n]);
            this.tweenScroll[n].y = this.origScrollTop[n] =
              this.getTargetScrollTop(this.$target[n]);
            this.handsfree.TweenMax.killTweensOf(this.tweenScroll[n]);
          }

          if (
            ((_hands$pinchState$n2 = hands.pinchState[n]) === null ||
            _hands$pinchState$n2 === void 0
              ? void 0
              : _hands$pinchState$n2[0]) === "held" &&
            this.$target[n]
          ) {
  ...
  var pluginPinchers = {
    ...
    onFrame({ hands }) {
      ...
      const height = this.handsfree.debug.$canvas.hands.height;
      const leftVisible = hands.multiHandedness.some(
        (hand) => hand.label === "Right"
      );
      const rightVisible = hands.multiHandedness.some(
      ...
      for (let n = 0; n < hands.multiHandLandmarks.length; n++) {
        ...
        for (let finger = 0; finger < 4; finger++) {
          ...
          const a =
            hands.multiHandLandmarks[n][4].x -
            hands.multiHandLandmarks[n][this.fingertipIndex[finger]].x;
          const b =
            hands.multiHandLandmarks[n][4].y -
            hands.multiHandLandmarks[n][this.fingertipIndex[finger]].y;
          const c = Math.sqrt(a * a + b * b) * height;
          const thresholdMet = (this.thresholdMet[hand][finger] =
            c < this.config.threshold);
          ...
          if (thresholdMet) {
            ...
            if (
              ...
              this.config.numThresholdErrorFrames
            ) {
      ...
      this.handsfree.data.hands = this.getPinchStates(
        hands,
        leftVisible,
        rightVisible
      );
      ...
      for (let hand = 0; hand < 2; hand++) {
        for (let finger = 0; finger < 4; finger++) {
          ...
          if (visible[hand] && this.thresholdMet[hand][finger]) {
            ...
            document.body.classList.add(
              `handsfree-finger-pinched-${hand}-${finger}`,
              `handsfree-finger-pinched-${finger}`
            );
          } else {
          ...
          if (
            this.pinchDowned[hand][finger] > 0 &&
            this.pinchDowned[hand][finger] <= this.config.maxMouseDownedFrames
          ) {
          ...
          if (hands.pinchState[hand][finger]) {
            ...
            this.handsfree.emit(
              `finger-pinched-${hands.pinchState[hand][finger]}-${hand}-${finger}`,
              {
                event: hands.pinchState[hand][finger],
                origPinch: hands.origPinch[hand][finger],
                curPinch: hands.curPinch[hand][finger],
            ...
            this.handsfree.emit(
              `finger-pinched-${hands.pinchState[hand][finger]}-${finger}`,
              {
                event: hands.pinchState[hand][finger],
                origPinch: hands.origPinch[hand][finger],
                curPinch: hands.curPinch[hand][finger],
    ...
    onFrame({ hands }) {
      ...
      hands.multiHandLandmarks.forEach((landmarks, n) => {
        ...
        this.handsfree.TweenMax.to(this.tween[hand], 1, {
          x:
            window.outerWidth * this.config.speed.x -
            (window.outerWidth * this.config.speed.x) / 2 +
            window.outerWidth / 2 -
            hands.multiHandLandmarks[n][21].x *
              this.config.speed.x *
              window.outerWidth +
            this.config.offset.x,
          y:
            hands.multiHandLandmarks[n][21].y *
              window.outerHeight *
              this.config.speed.y -
        ...
        let event =
          pointer === null || pointer === void 0
            ? void 0
            : (_pointer$pinchState = pointer.pinchState) === null ||
              _pointer$pinchState === void 0
            ? void 0
            : (_pointer$pinchState$n = _pointer$pinchState[n]) === null ||
              _pointer$pinchState$n === void 0
            ? void 0
            : _pointer$pinchState$n[0];
  ...
  */
  ...
  class Handsfree {
      ...
      this.on("modelReady", () => {
        ...
        if (++this.numModelsLoaded === numActiveModels) {
          ...
          if (
            ...
            (!this.isUpdating || (this.isUpdating && this.config.autostart))
          ) {
            this.isLooping = true;
            this.loop();
          }
    ...
    cleanConfig(config, defaults) {
      ...
      config.gesture &&
        Object.keys(config.gesture).forEach((gesture) => {
    ...
    start(callback) {
      ...
      Object.keys(this.config.plugin).forEach((plugin) => {
        ...
        if (
          typeof ((_this$config$plugin = this.config.plugin) === null ||
          _this$config$plugin === void 0
            ? void 0
            : (_this$config$plugin$p = _this$config$plugin[plugin]) === null ||
              _this$config$plugin$p === void 0
            ? void 0
            : _this$config$plugin$p.enabled) === "boolean" &&
      ...
      Object.keys(this.config.gesture).forEach((gesture) => {
        ...
        if (
          typeof ((_this$config$gesture = this.config.gesture) === null ||
          _this$config$gesture === void 0
            ? void 0
            : (_this$config$gesture$ = _this$config$gesture[gesture]) ===
                null || _this$config$gesture$ === void 0
            ? void 0
            : _this$config$gesture$.enabled) === "boolean" &&
          this.config.gesture[gesture].enabled
        ) {
          this.gesture[gesture].enable();
        }
      });
    ...
    loop() {
      ...
      (_this$taggedPlugins$u = this.taggedPlugins.untagged) === null ||
      _this$taggedPlugins$u === void 0
        ? void 0
        : _this$taggedPlugins$u.forEach((pluginName) => {
            var _this$plugin$pluginNa;

            this.plugin[pluginName].enabled &&
              ((_this$plugin$pluginNa = this.plugin[pluginName]) === null ||
              _this$plugin$pluginNa === void 0
                ? void 0
                : _this$plugin$pluginNa.onFrame(this.data));
          }); // Render video behind everything else
      // - Note: Weboji uses its own camera

      if (this.isDebugging) {
        const isUsingCamera = ["hands", "pose", "handpose", "facemesh"].find(
          (model) => {
    ...
    runPlugins(data) {
      ...
      (_this$taggedPlugins$u2 = this.taggedPlugins.untagged) === null ||
      _this$taggedPlugins$u2 === void 0
        ? void 0
        : _this$taggedPlugins$u2.forEach((pluginName) => {
            var _this$plugin$pluginNa2;

            this.plugin[pluginName].enabled &&
              ((_this$plugin$pluginNa2 = this.plugin[pluginName]) === null ||
              _this$plugin$pluginNa2 === void 0
                ? void 0
                : _this$plugin$pluginNa2.onFrame(this.data));
          });
    }
    /* //////////////////////// #4 GESTURES /////////////////////////
    ...
    getUserMedia(callback) {
      ...
      if (!this.debug.stream && !this.debug.isGettingStream) {
        ...
        if (
          this.isUsingWebcam &&
          (_this$model$weboji = this.model.weboji) !== null &&
          _this$model$weboji !== void 0 &&
          (_this$model$weboji$ap = _this$model$weboji.api) !== null &&
          _this$model$weboji$ap !== void 0 &&
          _this$model$weboji$ap.get_videoStream
        ) {
          this.debug.$video = this.model.weboji.api.get_video();
          this.debug.$video.srcObject = this.debug.stream =
            this.model.weboji.api.get_videoStream();
    ...
    setupDebugger() {
      ...
      ["video", "weboji", "facemesh", "pose", "hands", "handpose"].forEach(
        (model) => {
          ...
          this.debug.$canvas[model].width =
            this.config.setup.canvas[model].width;
          this.debug.$canvas[model].height =
            this.config.setup.canvas[model].height;
          ...
          if (["weboji", "handpose"].includes(model)) {
            ...
            this.debug.context[model] =
              this.debug.$canvas[model].getContext("2d");
          }
        }
